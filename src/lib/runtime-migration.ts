...
export async function ensureUserColumns(force: boolean = false): Promise<void> {
  ...
  const statements = [
    ...
    // 14. Integration table (Phase 4.3)
    ...
    // 15. UserSettings.currentWorkspaceId column (Phase 4.4)
    `ALTER TABLE "UserSettings"
       ADD COLUMN IF NOT EXISTS "currentWorkspaceId" TEXT;`,

    // 16. Workspace table (Phase 4.4)
    ...
    // 17. WorkspaceMember table (Phase 4.4)
    ...
    // 18. WorkspaceInvitation table (Phase 4.4)
